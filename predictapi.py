...
model = joblib.load(".venv/sign_model.pkl")
le = joblib.load(".venv/label_encoder.pkl")

base_cols  = [f"{ax}{i}" for i in range(21) for ax in ["x","y","z"]]
extra_cols = ([f"tip_wrist_{i}" for i in range(5)] +
              [f"tip_dist_{i}_{j}" for i in range(5) for j in range(i+1,5)] +
              [f"curl_{i}" for i in range(5)] + ["palm_ratio"])
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            result = predict_hand(hand)

            text = f"{result['letter']} ({result['confidence']:.0f}%)"
            color = (0, 220, 0)
